...
const addJingleListValidator = [
  check("name")
    ...
    .custom((value) => {
      return db.List.findOne({ where: { name: value } }).then((user) => {
        if (user) {
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (validationErrors.isEmpty()) {
      ...
      let addJingleListError = validationErrors
        .array()
        .map((error) => error.msg);
...
router.get(
  "/:userId(\\d+)/jingleLists/:jingleListId(\\d+)",
  csrfProtection,
  asyncHandler(async (req, res, next) => {
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const jingleListsToDestroy = await db.Jinglelist.findAll({
      where: { listId },
    });
...
router.post(
  "/:userId(\\d+)/jingleLists/:jingleListId(\\d+)/jingles/:jingleId(\\d+)",
  asyncHandler(async (req, res, next) => {
    const userId = req.params.userId;
    const listId = req.params.jingleListId;
...
router.post(
  "/:userId(\\d+)/jingleLists/:jingleListId(\\d+)/:jingleId",
  csrfProtection,
  asyncHandler(async (req, res, next) => {
    const jingleId = req.params.jingleId;
